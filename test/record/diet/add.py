...
def add_diet_record(i):
    ...
    try:
        ...
        if response.status_code == 200 and response.json().get("success", False):
            ...
            with lock:
                fail_count += 1
        print(f"添加diet: 状态码: {response.status_code}, 用时: {elapsed:.3f}s, 返回: {response.text}")
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        executor.map(add_diet_record, range(max_tasks))
    ...
    print(f"完成{max_tasks}次diet添加，成功{success_count}次，失败{fail_count}次。")
