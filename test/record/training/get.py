...
def get_training_record(i):
    ...
    try:
        ...
        if response.status_code == 200 and resp_json.get("success", False):
            ...
            with lock:
                fail_count += 1
        print(f"获取training: 状态码: {response.status_code}, 用时: {elapsed:.3f}s, 返回: {response.text}")
    except Exception as e:
        elapsed = time.time() - start
        with lock:
            fail_count += 1
            time_list.append(elapsed)
        print(f"获取training异常: {e}, 用时: {elapsed:.3f}s")
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        executor.map(get_training_record, range(max_tasks))
    ...
    print(f"完成{max_tasks}次training获取，成功{success_count}次，失败{fail_count}次。")
    print(f"总耗时: {total_time:.2f}s，平均单次耗时: {avg_time:.3f}s，最大耗时: {max_time:.3f}s，最小耗时: {min_time:.3f}s")
    print(f"QPS（每秒处理请求数）: {qps:.2f}")

    # 保存token和record_id
    with open("training_record_ids.json", "w", encoding="utf-8") as f:
        json.dump(record_id_list, f, ensure_ascii=False, indent=2)
    print(f"已保存 {len(record_id_list)} 条training记录到 training_record_ids.json")