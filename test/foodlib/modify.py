...
url = "https://api.fanovian.cc:3000/api/foodlib/modify"  # 或 trainlib/modify
...
def modify_item(i):
    ...
    try:
        ...
        with lock:
            fail_count += 1
            time_list.append(elapsed)
        print(f"修改异常: {e}, 用时: {elapsed:.3f}s")
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        executor.map(modify_item, range(max_tasks))
    ...
    print(f"完成{max_tasks}次修改，成功{success_count}次，失败{fail_count}次。")
