...
def get_profile(i):
    ...
    try:
        ...
        if response.status_code == 200 and response.json().get("success", False):
            ...
            with lock:
                fail_count += 1
        # print(f"获取profile: 状态码: {response.status_code}, 用时: {elapsed:.3f}s, 返回: {response.text}")
    except Exception as e:
        elapsed = time.time() - start
        with lock:
            fail_count += 1
            time_list.append(elapsed)
        print(f"获取profile异常: {e}, 用时: {elapsed:.3f}s")
...
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        executor.map(get_profile, range(max_tasks))
    ...
    print(f"完成{max_tasks}次profile获取，成功{success_count}次，失败{fail_count}次。")
